"""End-to-end PPO throughput benchmark for the fused Metal point env."""
...
from metalenvpool import MetalPointPool, PointConfig, resolve_device, synchronize
...
def main() -> None:
    ...
    pool = MetalPointPool(
        PointConfig(num_envs=args.num_envs, world_size=args.world_size, max_steps=args.max_episode_steps),
        device=device,
        use_shader=True,
    )
    ...
    actions = torch.empty((args.num_steps, args.num_envs, action_dim), device=device)
    ...
    out = {
        ...
        "mean_rollout_reward": float(rewards.mean().cpu()),
    }
